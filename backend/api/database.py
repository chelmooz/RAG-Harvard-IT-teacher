...
async def init_db() -> asyncpg.Pool:
    ...
    # le type PostgreSQL 'vector' (list[float] ↔ vector(1024)).
    ...
    async with _pool.acquire() as conn:
        ...
        await conn.execute("""
            CREATE TABLE IF NOT EXISTS rag_chunks (
                ...
                -- 1024 dims : BAAI/bge-m3 (remplace paraphrase-multilingual-
                -- mpnet-base-v2, 768 dims — espaces vectoriels incompatibles)
        ...
        # ── Garde de migration : dimension embedding ─────────────────────────
        # CREATE TABLE IF NOT EXISTS ne touche PAS une table déjà créée avec
        # l'ancienne dimension (768, paraphrase-multilingual-mpnet). Si une
        # telle table existe déjà, échouer bruyamment plutôt que de laisser
        # les futurs INSERT planter avec une erreur pgvector obscure — ou
        # pire, réussir silencieusement si l'ancienne colonne n'a jamais été
        # peuplée (0 lignes).
