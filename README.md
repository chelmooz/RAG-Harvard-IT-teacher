Prof-IA-v5-Documentation-BC250
# Prof IA v5.
...
```
```
LLM Runtime Ollama (image ROCm) Latest ROCm Inference Mistral 7B
```
```
...
```
```
amdgpu.gttsize 12288 (GRUB) Alloue 12 Go de VRAM sur les 16 Go
GDDR6 partagés
```
...
```
```
Ollama (ROCm) ollama/ollama:rocm Latest ROCm 11434
```
```
...
```
▸ GRUB : ajouter amdgpu.gttsize=12288 à GRUB_CMDLINE_LINUX_DEFAULT + update-grub
▸ Groupes utilisateur : sudo usermod -aG video,render $USER (requis pour /dev/kfd et /dev/dri)
▸ Service Ollama : /etc/systemd/system/ollama.service avec HSA_OVERRIDE_GFX_VERSION=10.1.
