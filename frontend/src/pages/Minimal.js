...
function Header({ nav, brandStyle, returnStyle, headerStyle }) {
  return (
...
function CenterContent({ input, setInput, send }) {
  return (
...
export default function Minimal() {
  ...
  return (
    <div style={pageStyle}>
      <Header nav={nav} brandStyle={brandStyle} returnStyle={returnStyle} headerStyle={headerStyle} />
      <div style={{ height: 1, background: 'linear-gradient(90deg, transparent, rgba(0,229,204,0.1), transparent)' }} />
      <CenterContent input={input} setInput={setInput} send={send} />
