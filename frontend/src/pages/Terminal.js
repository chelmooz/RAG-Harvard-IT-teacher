...
const labelText = { fontSize: 12, color: 'rgba(255,255,255,0.5)' };
...
export default function Terminal() {
  ...
  const send = async () => {
    ...
    try {
      const data = await chatService.sendMessage(q);
      const lines = [
        'ctOS_RAG >',
        data.response,
        '',
  ...
  const lineColor = (j, line) => j === 0
    ? '#00e5cc'
    : line.startsWith('>') ? 'rgba(255,255,255,0.75)' : 'rgba(255,255,255,0.55)';
  const inputBarStyle = {
    padding: '16px 20px',
